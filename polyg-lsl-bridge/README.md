...
| 옵션 | 의미 |
|---|---|
| `--log-values` | 채널별 **µV**(스케일 적용 후) 로깅 켜기 (기본 꺼짐) |
| `--log-raw` | 채널별 **raw 입력 전압(V, 스케일 적용 전)** 범위 로깅 — 스케일/포화 진단용 |
| `--log-interval SECONDS` | 로그 줄 사이 최소 간격(초). 기본 `0` = 매 프레임. 값을 키우면 스로틀 |

> ⚠️ 매 프레임 로그라 출력이 빠릅니다(예: PolyG-A 512Hz면 초당 약 32줄). 길게 측정할
> 때는 `--log-interval 0.5`(0.5초마다 한 줄)처럼 간격을 주세요. 이 로깅은 LSL 스트림
> 데이터 자체에는 영향을 주지 않습니다(콘솔 출력일 뿐).

#### µV 값이 이상할 때 — `--log-raw`로 진단

µV가 수십이 아니라 수십만(예: `294126`)으로 찍히면, 보통 두 원인입니다:

1. **`[scale].fixed_gain`이 placeholder `1.0`** — 전치증폭기 고정 게인을 사양/캘리브레이션
   값으로 채워야 합니다. (PGA 표는 0.1~17로 작아서 `gain_idx`만으론 배율이 수백~수천 배
   틀어지는 걸 설명 못 합니다.)
2. **입력 포화** — 전극이 떠 있으면(미연결) 고게인 프런트엔드가 ±1.25 V 레일까지 saturate.

`--log-raw`로 **스케일 적용 전 raw 전압**을 보면 둘을 구분할 수 있습니다:

```bash
polyg-bridge --config config.toml --log-raw --log-interval 0.5
#  raw V (*=railed near +-1.25 V) Fp1=[-0.002..+0.003] Fp2=[+1.248..+1.250]* ...
```

- 채널 값이 `[+1.248..+1.250]*`처럼 **`*`(±1.25 V 레일)에 붙어 있으면 포화** → 전극 연결을
  확인하세요(피험자에 부착 또는 입력 short 후 값이 줄면 포화였던 것).
- 레일이 아닌데도 µV가 너무 크면 **순수 스케일 문제** → `fixed_gain`을 설정하세요.
  알려진 진폭의 보정 신호 raw 전압을 측정하면 `fixed_gain = raw_V / (실제_µV × pga_gain × 1e-6)`
  으로 역산할 수 있습니다.

> `--log-values`(µV)와 `--log-raw`(V)는 함께 켤 수 있습니다.
